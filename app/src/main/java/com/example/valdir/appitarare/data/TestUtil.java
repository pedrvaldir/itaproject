...
public class TestUtil {
    ...
    public static int ANUN_WTSAPP = 0;
        ...
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_WHATSAPP, ANUN_WTSAPP_OK);
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_WIFI, ANUN_WI);
        list.add(cv);

        cv = new ContentValues();
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_TITULO, "FARMACIA SOUSA");
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_DESCRICAO, "TODO MEDICAMENTO VC ENCONTRA AQUI");
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_HORA_ATEND, "24H 7 VEZES POR SEMANA");
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_FORMA_PAGAMENTO, "CARTÕES: MASTERCARD, ELO, VISA");
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_AVALIADO, 10);
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_TEL_CONTATO, "(15) 4567-9844");
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_IMG, R.drawable.empresa1);
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_WHATSAPP, ANUN_WTSAPP_OK);
        cv.put(AnuncioContract.AnuncioEntrada.COLUNA_WIFI, ANUN_WI);
        ...
        try
        {
            ...
            db.delete (AnuncioContract.AnuncioEntrada.NOME_TABELA,null,null);
            //go through the list and add one by one
            for(ContentValues c:list){
                db.insert(AnuncioContract.AnuncioEntrada.NOME_TABELA, null, c);
            }
